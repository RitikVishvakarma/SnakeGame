...
public class Board extends JPanel implements ActionListener{
    ...
    Board(){
        addKeyListener(new TAdapter()); // use for right, left, up, down key
        setBackground(Color.BLACK);
        setPreferredSize(new Dimension(300,300));
        setFocusable(true); // use of addKeyListener(new TAdapter())
        loadImages();
        initGame();
    }
    ...
    public void locateApple(){
        int r=(int) (Math.random()*RANDOM_POSITION); //0 and 1 = 0.6*20=12*10(DOT_SIZE)=120
        apple_x=(r*DOT_SIZE);
        r=(int) (Math.random()*RANDOM_POSITION); //0 and 1 = 0.6*20=12*10(DOT_SIZE)=120
        apple_y=(r*DOT_SIZE);
